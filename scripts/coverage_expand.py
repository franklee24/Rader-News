...
AUTH = {"reuters.com":1.0,"apnews.com":.95,"bbc.com":.9,"bbc.co.uk":.9,"nytimes.com":.9,"ft.com":.95,"bloomberg.com":.95,"theguardian.com":.85,"cnn.com":.8,"npr.org":.8,"nhk.or.jp":.9,"yonhapnews.co.kr":.9,"xinhua.net":.9,"gov.cn":1.0,"gov.uk":1.0,"elysee.fr":1.0,"bundesregierung.de":1.0,"kremlin.ru":1.0,"japan.go.jp":1.0}
...
def norm(s):
    s=re.sub(r'https?://\\S+',' ',(s or '').lower()); s=re.sub(r'[^a-z0-9\\u4e00-\\u9fff]+',' ',s); return ' '.join(s.split())
def sim(a,b):
...
def fetch(query,start,end,maxrecords=160):
    params={'query':query,'mode':'ArtList','format':'json','maxrecords':str(maxrecords),'sort':'DateDesc','startdatetime':start.strftime('%Y%m%d%H%M%S'),'enddatetime':end.strftime('%Y%m%d%H%M%S')}
    req=Request(API+'?'+urlencode(params),headers={'User-Agent':'Leida-News-Coverage/1.0'})
    with urlopen(req,timeout=35) as r:return json.loads(r.read().decode('utf-8','replace')).get('articles',[])

def main():
    data=json.loads(DAILY.read_text(encoding='utf-8'))
    bj=timezone(timedelta(hours=8)); now=datetime.now(timezone.utc).astimezone(bj); anchor=now.replace(hour=8,minute=0,second=0,microsecond=0)
    ...
    for tier,countries in CFG['countries'].items():
        ...
        for c in countries:
            name=c['name']; current=bucket.get(name,{ }); events=current.get('events',[]); target=min(c.get('max',10),tier_targets.get(tier,8))
            if len(events)>=target:continue
            aliases=ALIASES.get(name,[c.get('en',name),name]); alias_q=' OR '.join('"'+x+'"' for x in aliases); candidates={}
            ...
            for a in candidates.values():
                title=a.get('title',''); domain=a.get('domain','unknown'); high=sum(x.lower() in title.lower() for x in HIGH); ranked.append((35+domain_score(domain)*25+high*7,a))
            ranked.sort(key=lambda x:(x[0],x[1].get('seendate','')),reverse=True)
            for _,a in ranked:
                if len(events)>=target:break
                title=a.get('title','').strip()
                if any(sim(title,e.get('title',''))>=.55 for e in events):continue
                domain=a.get('domain') or 'unknown'; url=a.get('url') or a.get('url_mobile') or ''; lang=(a.get('language') or '').lower(); published=a.get('seendate') or ''; high=sum(x.lower() in title.lower() for x in HIGH)
                importance=round(min(100,35+domain_score(domain)*25+high*7),1); eid=hashlib.sha1((c['code']+'|'+norm(title)).encode()).hexdigest()[:16]
                events.append({'id':eid,'title':title,'published_at':published,'code':c['code'],'event_country':name,'sources':[{'title':title,'link':url,'published_at':published,'source':domain,'source_url':url,'language':lang}],'source':domain,'url':url,'source_url':url,'source_count':1,'source_names':[domain],'category':'政治','domestic_score':35,'importance':importance,'why_important':'该事件在过去24小时内形成具有实际政策、经济、安全、产业或社会影响的新闻信号。','impact':'关注相关政策、市场、产业链、安全局势及国际关系的后续影响。','next_72h':'关注官方公告、市场反应及其他权威媒体的独立确认。','importance_level':'值得关注' if importance>=60 else '一般动态','importance_model':'V20：事件级动态评分；覆盖扩展补充'}); added+=1
            current['events']=events; current['count']=len(events); bucket[name]=current
    ...
    DAILY.write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8'); HISTORY.mkdir(parents=True,exist_ok=True); (HISTORY/f'{anchor.date().isoformat()}.json').write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8')
