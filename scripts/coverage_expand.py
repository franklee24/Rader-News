...
DAILY=ROOT/'data/daily.json'; HISTORY=ROOT/'data/history'; API='https://api.gdeltproject.org/api/v2/doc/doc'
    ...
    for tier,countries in CFG['countries'].items():
        ...
        for c in countries:
            ...
            for _,a in ranked:
                ...
                domain=a.get('domain') or 'unknown'; url=a.get('url') or a.get('url_mobile') or ''; lang=(a.get('language') or '').lower(); published=a.get('seendate') or ''; high=sum(x.lower() in title.lower() for x in HIGH); importance=round(min(100,35+domain_score(domain)*25+high*7),1); eid=hashlib.sha1((c['code']+'|'+norm(title)).encode()).hexdigest()[:16]
